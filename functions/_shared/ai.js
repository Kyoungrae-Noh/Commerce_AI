...
9. 현재 월(\${new Date().getMonth() + 1}월)을 기준으로 계절성을 판단하세요. 소싱은 실제 수요보다 1~2달 앞서야 하므로, 아래 기준으로 판단하세요. 계절과 명확히 관련된 상품만 계절성을 언급하고, 강아지 간식, 생활 소모품, 전자기기 등 계절과 무관한 상품은 계절성 언급을 절대 하지 마세요. 딸기/복숭아/수박 등 과일은 카테고리별 제철 시즌을 정확히 반영하세요. (딸기=봄/겨울, 수박=여름, 복숭아=여름)
